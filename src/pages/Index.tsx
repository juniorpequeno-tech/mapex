...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      {/* Top bar */}
      <div className="border-b border-border px-2 sm:px-4 py-2 sm:py-3 shrink-0">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            <button
              onClick={() => navigate('/')}
              className="h-7 w-7 rounded hover:bg-accent flex items-center justify-center text-muted-foreground shrink-0"
              title="Voltar"
            >
            ...
            {editingName && canEdit ? (
              <Input
                className="h-7 w-32 sm:w-48 text-sm"
                value={fileName}
              ...
              <button
                className="text-sm font-medium hover:text-primary flex items-center gap-1 transition-colors truncate min-w-0"
                onClick={() => canEdit && setEditingName(true)}
              >
                <span className="truncate">{fileName}</span>
                {canEdit && <Pencil className="h-3 w-3 text-muted-foreground shrink-0" />}
              </button>
            )}
            {permissionBadge}
          </div>
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            ...
            {canShare && (
              <Button
                size="sm"
                variant="outline"
                className="gap-1 sm:gap-1.5 text-xs h-7 sm:h-8 px-2 sm:px-3"
            ...
            {canEdit && (
              <button
                onClick={handleSave}
                className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 text-xs font-medium bg-primary text-primary-foreground rounded hover:opacity-90 transition-opacity"
              >
